                            ...
                            <div class="theme-form">
                                <div class="row check-out ">
                                    ...
                                    <div class="form-group col-md-12 col-sm-6 col-xs-12">
                                        ...
                                        <label class="field-label">ডেলিভারি কোথায় এবং কিভাবে নিবেন?</label><br/>
                                        ...
                                        <input type="radio" class="delivery-type" name="delivery_type" id="delivery-type-{{$type->id}}" data-type="{{$type->id}}" data-charge="{{$type->price}}" value="{{$type->id}}" @if($del_type==$type->id) checked @endif required> {{$del_bangs[$type->name]}} - &#2547;{{\App\Http\Controllers\HomeController::bfn(number_format($type->price))}}<br/>
                                        @endforeach
                                    </div>
                                ...
                                <div class="order-box">
                                    ...
                                    {{-- <ul class="qty"> --}}
                                        ...
                                        @endphp
                                            ...
                                            <div class="col-3 cart-item-row text-right">&#2547;<span id="{{$id}}-subtotal" class="subtotal d-none">{{number_format($details['price'] * $details['quantity'],2,".","")}}</span><span id="{{$id}}-subtotal_show" class="subtotal">{{\App\Http\Controllers\HomeController::bfn(number_format($details['price'] * $details['quantity'],2,".",""))}}</span></div>
                                        @endforeach
                                        ...
                                        @php
                                            ...
                                            if(session('delivery')){
                                                ...
                                                $deliveryInfo = \App\Deliverypackages::find($delivery['type']);
                                                $delivery_charge  = $deliveryInfo->price;

                                            }else if(isset(Auth::user()->defaut_delivery_option) && Auth::user()->defaut_delivery_option != NULL){

                                                $deliveryInfo = \App\Deliverypackages::find(Auth::user()->defaut_delivery_option);
                                                $delivery_charge  = $deliveryInfo->price;

                                            }else{
                                                $deliveryInfo = \App\Deliverypackages::find(3);
                                                $delivery_charge  = $deliveryInfo->price;
                                            }
                                        ...
                                        <div class="col-6 cart-item-row">ডেলিভারি চার্জ</div><div class="col-6 cart-item-row text-right"><span class="count">&#2547;<span id="delivery_charge" class="d-none">{{number_format($delivery_charge,2,".","")}}</span><span id="delivery_charge_show">{{\App\Http\Controllers\HomeController::bfn(number_format($delivery_charge,2,".",""))}}</span></span></div>
                                        <div class="col-6 cart-item-row">ডিস্কাউন্ট</div><div class="col-6 cart-item-row text-right"><span class="discount">&#2547;<span id="discount_amount" class="d-none">{{number_format($discount,2,".","")}}</span><span id="discount_amount_show">{{\App\Http\Controllers\HomeController::bfn(number_format($discount,2,".",""))}}</span></span><input type="hidden" name="discount" id="discount" class="form-control" value="{{$discount}}"></div>
                                        <div class="col-6 cart-item-row">সর্বমোট</div><div class="col-6 cart-item-row text-right"><span class="count">&#2547;<span id="total" class="d-none">{{number_format($grand_total,2,".","")}}</span><span id="total_show">{{\App\Http\Controllers\HomeController::bfn(number_format($grand_total,2,".",""))}}</span></span></div>
